# =============================================================================
#   scripts\pure_metadata_export.py
# =============================================================================
"""
DB용 순수 메타데이터 JSONL 변환·검증.

- 데이터셋 파이프라인 기본 산출물은 metadata.jsonl (이미 순수 필드만 있음).
- 예전 dataset.jsonl / finetune_dataset.jsonl(output 블록)에서 DB 스키마만 뽑을 때 사용.

사용 (저장소 루트 ``ollama-api/`` 에서):

  python scripts/pure_metadata_export.py
  python scripts/pure_metadata_export.py --in metadata/finetune_dataset.jsonl --out metadata/import.jsonl
  python scripts/pure_metadata_export.py --in metadata/dataset.jsonl  # 레거시

입력이 없으면 루트의 metadata.jsonl, 없으면 dataset.jsonl을 찾는다.
"""
